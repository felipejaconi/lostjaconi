const fs = require('fs');
let code = fs.readFileSync('src/pages/admin/AdminStoreSales.tsx', 'utf8');
...
const targetDiv = '<div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">';
...
code = code.replace(
  /<div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">\s*<div>\s*<BrandTitle title="C\.M\.V\." hideUnderline titleClassName="max-md:mt-0 md:-mt-4 max-md:pl-0 max-md:pt-0 max-md:ml-0 !mb-1" \/>\s*<\/div>/,
  replacement
);

fs.writeFileSync('src/pages/admin/AdminStoreSales.tsx', code);
